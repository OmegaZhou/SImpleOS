#include "include/file_sys_config.h"
...
void add_item(int father_id, int son_id)
{
	...
	while (header->used_size + ID_SIZE > SECTOR_SIZE) {
		if (header->next_id != -1) {
			...
			int id = add_file_sector(header->type, header->father_id, header->now_id, (char*)header + 1, header);
...
int check_file_exist(int father_id, char* name, int* get_id)
{
	...
	if (str_comp(name, ".")==0 || str_comp(name, "..")==0) {
		return 1;
	}
